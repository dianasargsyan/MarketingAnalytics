...
def layout3(data):
    ...
    dbc.DropdownMenuItem("1st Day Activity",id = "day_tab3"),
    dbc.DropdownMenuItem("7 Day Activity",id = "day7_tab3"),
    ...
    # df = pd.crosstab(df['cost'], df['group']).reindex(columns=a, index=a, fill_value=0)#.reset_index()
...
@app.callback(
    ...
    [Input('gain_tab3','n_clicks'),
     Input('day_tab3','n_clicks'),
     Input('day7_tab3','n_clicks'),
...
def display_table(*args):
   ...
   # if button_id == 'cross'
    if button_id == 'gain_tab3':
        target = 'gain'
    elif button_id == 'day_tab3':
        target = 'day'
    elif button_id == 'day7_tab3':
        target = 'day7'
    elif button_id == 'cost_tab3':
        target = 'cost'
    d1 = pd.crosstab(data['group'],data[target])
    d1.to_csv("a.csv")
